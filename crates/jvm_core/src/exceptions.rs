...
pub fn build_stack_trace_frames(
...
fn make_local_variable(
    ...
    let name;
    unsafe {
        name = jni_env.new_string_utf(entry.name)?;
    }

    let local = if location >= entry.start_location
        && location <= entry.start_location + i64::from(entry.length)
    {
        get_local_value(jvmti_env, jni_env, thread, depth, entry)
            .and_then(|value| jni_env.new_object_StringL(local_class, ctor, name, value))
