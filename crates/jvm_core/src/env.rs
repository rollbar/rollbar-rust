...
use crate::jvmti::{
    ...
    jvmtiLocalVariableEntry, JNIEnv, JavaVM, JNI_ERR, JNI_OK, JVMTI_VERSION,
};

pub type ExceptionCallbackFn = unsafe extern "C" fn(
    *mut jvmtiEnv,
    *mut JNIEnv,
    jthread,
    jmethodID,
...
impl JvmTiEnv {
    pub unsafe fn maybe_new(vm: *mut JavaVM) -> Result<JvmTiEnv> {
        let mut penv: *mut c_void = ptr::null_mut();
        let rc =
            (**vm).GetEnv.expect("GetEnv function not found")(vm, &mut penv, JVMTI_VERSION as i32);
        if rc as u32 != JNI_OK {
    ...
    pub fn enable_capabilities(&mut self, access_locals: bool) -> Result<()> {
    ...
    pub fn set_exception_handler(&mut self, callback: ExceptionCallbackFn) -> Result<()> {
        let callbacks = jvmtiEventCallbacks {
            Exception: Some(callback),
            ..Default::default()
        };
